...
while(rs.next()){
	...
	if(firstRec){
		firstRec = false;
		oldDep = rs.getDouble("CBS_DEPOSIT");
		oldWith = rs.getDouble("CBS_WITH");
		oldMonth = rs.getDate("CBS_DATE").getMonth(); 
		%>
		<tr>
		<td><%=rs.getDate("CBS_DATE") %></td>
		<td><%=rs.getString("CBS_DESC") %></td>
		<td><%=rs.getDouble("CBS_DEPOSIT") %></td>
		<td><%=rs.getDouble("CBS_WITH") %></td>
		<td><%=rs.getDouble("CBS_BAL") %></td>
		</tr>
		<%
	}else{
		newMonth = rs.getDate("CBS_DATE").getMonth();
		if(oldMonth==newMonth){
			oldDep = oldDep+rs.getDouble("CBS_DEPOSIT");
			oldWith = oldWith+rs.getDouble("CBS_WITH");
			//oldMonth = rs.getDate("CBS_DATE").getMonth(); 
			%>
			<tr>
			<td><%=rs.getDate("CBS_DATE") %></td>
